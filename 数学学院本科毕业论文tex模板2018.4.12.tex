...
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{CellImmune.eps}
\caption{特异性细胞免疫示意图}
\label{Cell_Immune}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{SerumImmune.eps}
\caption{特异性体液免疫示意图}
\label{SerumImmune}
\end{figure}

人类的抗体结构是一个二聚体，由两条重链(heavy chain)和两条轻链(light chain)组成。每条链又分为可变区(variable fragment)和不变区(constant fragment)。抗体的特异性则主要来自与可变区的互补决定区(CDR, complementarity determining region)如图。
\vspace{4cm}

互补决定区主要由三个比较短的氨基酸片段组成，它们分别称为CDR1, CDR2和CDR3。
